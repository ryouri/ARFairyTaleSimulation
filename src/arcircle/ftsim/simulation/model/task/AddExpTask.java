...
public class AddExpTask extends Task {
	...
	public AddExpTask(TaskManager taskManager, Chara chara, int addExp) {
		...
		try {
			levelWindow = new Image("./image/simple2.png");
	...
	private void calcPos(){
		//レベルアップ用Windowの表示位置
		lvWinPosX = CenterX - (levelWindow.getWidth() / 2);
		lvWinPosY = CenterY - (levelWindow.getHeight() / 2);
		//経験値上昇の記述位置
	...
	public void render(Graphics g, int offsetX, int offsetY, int firstTileX,
		...
		//レベルアップ用Windowの表示
		g.drawImage(levelWindow, lvWinPosX, lvWinPosY);
		//経験値上昇の記述描画
