...
public class SupportCommand extends Command implements KeyListner, Renderer {

	private Chara chara;
	private Field field;

	Image supportRangeImage;
	Color supportColor;
	...
	public SupportCommand(String commandName, SimGameModel sgModel,
			CharaCommandWindow charaCommandWindow) {
		super(commandName, sgModel, charaCommandWindow);
		try {
			supportRangeImage = new Image("image/commandWindow/supportRange.png");
		} catch (SlickException e) {
			e.printStackTrace();
		}
		supportColor = new Color(1, 1, 1, 0.5f);
	}
	...
	private void calculateJudgeAttack() {
		supportJudge = new boolean[supportRange.length][supportRange[0].length];
		boolean[][] charaPut = new boolean[supportRange.length][supportRange[0].length];
...
//				}
	...
	private void targetDamageChara(Chara targetChara) {
